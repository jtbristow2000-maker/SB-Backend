...
create index if not exists messages_customer_profile_created_idx
  on public.messages (customer_profile_id, created_at);
...
create index if not exists quote_drafts_business_status_idx
  on public.quote_drafts (business_id, status);

create index if not exists quote_drafts_customer_profile_idx
  on public.quote_drafts (customer_profile_id);
