...
// Settings — organized into tabbed sub-menus (SettingsShell) instead of one
// long scroll: Phone · AI & Replies · Voicemail · Business · Hours & Weather ·
// Services. All fields still live in ONE form (hidden tabs stay mounted), so
// saving from any tab captures everything.
...
export default async function SettingsPage() {
  ...
  const phoneTab = numberModel ? (
    ...
    <div style={S.sectionHint}>Phone setup is unavailable right now.</div>
  );
    ...
    <section style={S.section}>
      ...
      <label style={S.checkRow}>
        ...
        <span>
          ...
          <span style={S.checkHint}>On: callers ring your phone, then voicemail if you miss it. Off: callers go straight to voicemail. (Calls forwarded from your own number always go straight to voicemail.)</span>
        </span>
      </label>
  ...
  const servicesTab = (
    <section style={S.section}>
      <div style={S.sectionTitle}>Services &amp; prices</div>
      <div style={S.sectionHint}>Your menu — each service&apos;s price range, calendar color, and how long it takes. Powers quotes and offered times.</div>
