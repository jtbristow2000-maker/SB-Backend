...
- 2026-06-03: Phase 1 owner login + strict per-business isolation is in progress in `web/`:
  ...
  - IN PROGRESS (Codex, 2026-06-03): RLS policies are being added for tenant tables.
  - Supabase rollout note: apply `web/supabase/migrations/0004_business_members.sql` and `0005_business_rls.sql`, then enable Supabase Email auth before setting `PERSISTENCE=supabase` for owner login.
  - Next steps in this phase: owner route protection and owner read/action business-context replacement.
